    ...
    def get_forwards(self):
        ...
        return np.array(
            [
                np.cos(
                    np.radians(
                        self.eulers[1]),dtype=np.float32
                ) * np.cos(
                    np.radians(
                        self.eulers[0]
                    ),dtype=np.float32
...
class Engine:
    ...
    def render(self, scene):
        ...
        glUniform3fv(
            self.cameraForwardsLocation, 1, scene.player.get_forwards())
        glUniform3fv(
            self.cameraUpLocation, 1, 
            (SCREEN_HEIGHT / SCREEN_WIDTH) * scene.player.get_up())
        glUniform3fv(
...
"""
Practical 4:  Andrew Mengede (2023), <https://github.com/amengede/computerGraphicsPracs/tree/main/Prac4%20-%20Textures%20Cubemaps%20and%20Lighting>
"""
class Material:
...
class ObjMesh(Mesh):
    ...
    def loadMesh(self, filename,split:int):
        ...
        """ 
        Author:  Andrew Mengede (2023), <https://github.com/amengede/computerGraphicsPracs/tree/main/Prac4%20-%20Textures%20Cubemaps%20and%20Lighting>
        """
